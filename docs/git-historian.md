...
* Python 3.10+
* `requests` (installed automatically when you run `pip install -e .`)
...
* The script creates `docs/history/YYYY-MM-DD-checkin.md` using [`templates/weekly.md`](../templates/weekly.md).
...
* **Tip:** If you see `ModuleNotFoundError: No module named 'contextledger'`, ensure the package is installed in your environment (`python -m pip install -e .`).
...
3. Run `python -m contextledger.cli --since 7d --until now --output docs/history`.
...
* Use the machine-readable index at `docs/history/context-index.json` (generated by the script) to support RAG pipelines.
